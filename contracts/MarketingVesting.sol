...
contract TokenLocking {
    ...
    bool private releaseAtTimestamp;
    ...
    constructor(
        ...
        bool releaseAtTimestamp_
    ) {
        token = token_;
        start = start_;
        duration = duration_;
        initialReleasePercentage = initialReleasePercentage_;
        releaseAtTimestamp = releaseAtTimestamp_;
    ...
    function getReleasedTokensAtTimestamp(address beneficiary, uint256 timestamp) public view returns (uint256 amount) {
        if (timestamp < start) {
            return 0;
        }
    ...
    function _allocateTokens(address[] memory beneficiaries, uint256[] memory amounts) internal {
        require(
...
contract MarketingVesting is TokenLocking {
    bool internal constant _RELEASE_AT_TIMESTAMP = false;
    uint256 internal constant _INITIAL_RELEASE_PERCENTAGE = 0;
